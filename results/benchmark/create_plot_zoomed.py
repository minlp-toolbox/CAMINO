...
key = "nonconv"
scale = 160  # int(input("Amount (e.g. 120):"))
...
for s, name, style, color in zip(solvers, solver_names, styles, colors):
    ...
    collect_bins_plot(subdata[f"{s}.ratio_obj"], name, style, color, min_val=1, max_val=1e5, ax=axins)
...
plt.tight_layout()
plt.savefig(f"benchmark_comb_nonconv.pdf")
plt.show()
