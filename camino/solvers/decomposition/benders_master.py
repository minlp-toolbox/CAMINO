...
class BendersMasterMILP(SolverClass):
    ...
    def warmstart(self, data):
        ...
        if not data.relaxed:
            for solution in data.best_solutions:
                self.add_solution(
                    data, solution["x"], solution["lam_g"], solution["lam_x"], True
                )
    ...
    def setup_common(self, problem: MinlpProblem, s: Settings):
        ...
        self.options["discrete"] = np.ones_like(problem.idx_x_integer).tolist() + [0]
    ...
    def add_solution(self, nlpdata, x_sol, lam_g_sol, lam_x_sol, prev_feasible):
        ...
        g_k = self._generate_cut_equation(
            self._x,
            x_sol[: self.nr_x_orig],
            x_sol[self.idx_x_integer],
            lam_g_sol,
            lam_x_sol,
            nlpdata.p,
            nlpdata.lbg,
...
class BendersMasterMIQP(BendersMasterMILP):
    ...
    def __init__(
        ...
        self.f_hess_bin = ca.Function(
            ...
            [problem.x, problem.p],
            [
                ca.hessian(problem.f, problem.x)[0][problem.idx_x_integer, :][
                    :, problem.idx_x_integer
                ]
            ],
...
class BendersTrustRegionMIP(BendersMasterMILP):
    ...
    def __init__(
        ...
        self.f_hess = ca.Function(
            "hess_f_x", [problem.x, problem.p], [ca.hessian(problem.f, problem.x)[0]]
        )
    ...
    def solve(self, nlpdata: MinlpData, relaxed=False) -> MinlpData:
        ...
        g_k = self._generate_cut_equation(
            self._x[self.idx_x_integer],
            x_sol_prev,
            x_sol_prev[self.idx_x_integer],
            nlpdata.lam_g_sol,
            nlpdata.lam_x_sol,
            nlpdata.p,
            nlpdata.lbg,
        ...
        if g_lin.numel() > 0:
            jac_g = self.jac_g_sub(self.x_sol_best, nlpdata.p)[self.idx_g_nonlin, :]
        ...
        solver = ca.qpsol(
            f"benders_constraint_{self.nr_g}",
            self.settings.MIP_SOLVER,
