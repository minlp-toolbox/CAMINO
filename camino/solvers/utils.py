...
def get_solutions_pool(
    nlpdata, success, stats, s: Settings, current_solution, best_solution, idx_x_integer
) -> MinlpData:
    """Get pool of solutions if exists."""
    if (
        s.USE_SOLUTION_POOL
        and stats
        and "pool_sol_nr" in stats
        and stats["pool_sol_nr"]
    ):
        sols = []
        x_sols = []
        for i in range(stats["pool_sol_nr"]):
            x = ca.DM(stats["pool_solutions"][i])
            if i > 0 and np.allclose(
                to_0d(x)[idx_x_integer], to_0d(best_solution)[idx_x_integer]
            ):
                pass
            else:
        ...
        nlpdata.prev_solutions = [
            {"f": current_solution["f"], "x": current_solution["x"]}
        ]  # [nlpdata.prev_solutions[-1]]
    ...
    if termination_type == "gradient":
        ...
        def func(
            ...
            if ret:
                ...
                logger.info(
                    colored(
                        f"LB = {lb:.8f} | UB = {ub:.8f} | TOL = {tol_abs:.8f}", "green"
                    )
                )
            else:
                logger.info(colored(f"Not Terminated."))
                logger.info(
                    colored(f"LB = {lb:.3f} | UB = {ub:.3f} | TOL = {tol_abs:.3f}")
                )
            return max_time(ret, s, stats)
